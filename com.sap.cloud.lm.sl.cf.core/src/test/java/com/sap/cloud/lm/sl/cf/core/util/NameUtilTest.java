...
public class NameUtilTest {
    ...
    public void testIsValidEnvNameWhenNamesAreValid() {
        testIsValidName(Arrays.asList("_foo", "Foo", "foo_bar", "Foo_Bar", "x_1"), NameRequirements.ENVIRONMENT_NAME_PATTERN, true);
    }

    @Test
    public void testIsValidEnvNameWhenNamesAreInvalid() {
        String longName = repeat("f", NameRequirements.ENVIRONMENT_NAME_MAX_LENGTH + 1);

        testIsValidName(Arrays.asList("1foo", "   ", "foo bar", "foo-bar", "x&y", longName), NameRequirements.ENVIRONMENT_NAME_PATTERN,
            false);
    }
