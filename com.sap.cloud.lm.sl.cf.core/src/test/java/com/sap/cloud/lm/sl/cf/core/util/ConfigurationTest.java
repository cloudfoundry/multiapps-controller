...
public class ConfigurationTest {
    ...
    public void testGetSpaceGuidWithEmptyString() throws Exception {
        Mockito.when(environment.getVariable(ApplicationConfiguration.CFG_VCAP_APPLICATION)).thenReturn("");
        assertEquals(ApplicationConfiguration.DEFAULT_SPACE_ID, configuration.getSpaceGuid());
    }

    @Test
    public void testGetSpaceGuidWithInvalidJson() throws Exception {
        Mockito.when(environment.getVariable(ApplicationConfiguration.CFG_VCAP_APPLICATION)).thenReturn("invalid");
        assertEquals(ApplicationConfiguration.DEFAULT_SPACE_ID, configuration.getSpaceGuid());
    }
    ...
    public void testGetSpaceGuidWithMissingSpaceId() throws Exception {
        Mockito.when(environment.getVariable(ApplicationConfiguration.CFG_VCAP_APPLICATION)).thenReturn(VCAP_APPLICATION_JSON_WITHOUT_SPACE_GUID);
        assertEquals(ApplicationConfiguration.DEFAULT_SPACE_ID, configuration.getSpaceGuid());
    }

    @Test
    public void testGetSpaceGuid() throws Exception {
        Mockito.when(environment.getVariable(ApplicationConfiguration.CFG_VCAP_APPLICATION)).thenReturn(VCAP_APPLICATION_JSON);
