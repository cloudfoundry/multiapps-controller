...
import com.sap.cloud.lm.sl.cf.core.helpers.DefaultTagsDetector;
import com.sap.cloud.lm.sl.cf.core.helpers.ServiceCreator;
...
@Component("createServicesStep")
public class CreateServicesStep extends AbstractXS2ProcessStep {

    // Logger
    private static final Logger LOGGER = LoggerFactory.getLogger(CreateServicesStep.class);

    private SecureSerializationFacade secureSerializer = new SecureSerializationFacade();

    public static StepMetadata getMetadata() {
        return new StepMetadata("createServicesTask", "Create Services", "Create Services");
    }

    protected Function<DelegateExecution, CloudFoundryOperations> clientSupplier = (context) -> getCloudFoundryClient(context, LOGGER);

    protected Function<DelegateExecution, ClientExtensions> clientExtensionsSupplier = (context) -> getClientExtensions(context, LOGGER);
    ...
    protected ExecutionStatus executeStepInternal(DelegateExecution context) throws SLException, FileStorageException {
        ...
        try {
            info(context, Messages.CREATING_SERVICES, LOGGER);
            ...
            createServices(context, client, services, updatedServices, existingServicesMap, serviceKeys, defaultTags);
            StepsUtil.setUpdatedServices(context, updatedServices);

            debug(context, Messages.SERVICES_CREATED, LOGGER);
            ...
            error(context, Messages.ERROR_CREATING_SERVICES, ex, LOGGER);
            throw ex;
        }
    }
    ...
    private void createServices(DelegateExecution context, CloudFoundryOperations client, List<CloudServiceExtended> services,
        Set<String> updatedServices, Map<String, CloudService> existingServices, Map<String, List<ServiceKey>> serviceKeys,
        ...
        for (CloudServiceExtended service : services) {
            ...
            createService(context, client, service, updatedServices, existingService, deleteAllowed, defaultTagsForService);
    ...
    private void createOrUpdateServiceKeys(List<ServiceKey> serviceKeys, CloudServiceExtended service, CloudService existingService,
        ...
        List<ServiceKey> existingServiceKeys = clientExtensions.getServiceKeys(service.getName());
    ...
    private void createService(DelegateExecution context, CloudFoundryOperations client, CloudServiceExtended service,
        Set<String> updatedServices, CloudService existingService, boolean deleteAllowed, List<String> defaultTags)
        ...
        if (existingService != null) {
            ...
            if (actions.contains(ServiceAction.ACTION_RECREATE)) {
                if (deleteAllowed) {
                    ...
                    createService(context, client, service);
                    updatedServices.add(service.getName());
                } else {
            ...
            if (actions.contains(ServiceAction.ACTION_UPDATE_CREDENTIALS)) {
                updatedServices.add(service.getName());
                updateServiceCredentials(context, client, service);
            }
            if (actions.contains(ServiceAction.ACTION_UPDATE_TAGS)) {
                updatedServices.add(service.getName());
                updateServiceTags(context, client, service);
            }
            if (actions.isEmpty()) {
    ...
    private List<ServiceAction> determineActions(DelegateExecution context, CloudFoundryOperations client, CloudServiceExtended service,
        ...
        if (!existingService.isUserProvided() && existingService instanceof CloudServiceExtended) {
            CloudServiceExtended cswc = (CloudServiceExtended) existingService;
            if (shouldUpdateCredentials(service, cswc.getCredentials())) {
                ...
                debug(context, "Existing service parameters: " + secureSerializer.toJson(cswc.getCredentials()), LOGGER);
    ...
    private ClientExtensions getClientExtensions(DelegateExecution context) throws SLException {
        ClientExtensions clientExtensions = clientExtensionsSupplier.apply(context);
        if (clientExtensions == null)
