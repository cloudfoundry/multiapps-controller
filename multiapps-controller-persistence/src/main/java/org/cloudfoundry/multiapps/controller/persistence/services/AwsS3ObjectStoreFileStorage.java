...
public class AwsS3ObjectStoreFileStorage extends ObjectStoreFileStorage {
    ...
    protected S3Client createS3Client(Map<String, Object> credentials) {
        ...
        builder.endpointOverride(URI.create(resolveEndpoint(credentials)));
        builder.region(software.amazon.awssdk.regions.Region.of((String) credentials.get(CredentialKeys.REGION)));
        return builder.build();
    }

    private String resolveEndpoint(Map<String, Object> credentials) {
        String endpoint = (String) credentials.get(CredentialKeys.ENDPOINT);
        if (endpoint != null) {
            return endpoint;
        }
        return HTTPS_SCHEME + credentials.get(CredentialKeys.HOST);
    }
    ...
    private void batchDelete(List<String> keys) {
        for (int i = 0; i < keys.size(); i += DELETE_BATCH_SIZE) {
            ...
            for (S3Error error : response.errors()) {
                LOGGER.warn(MessageFormat.format(Messages.FAILED_TO_DELETE_FILE_0_IN_OBJECT_STORE_REASON_1, error.key(), error.message()));
            }
        }
    }
    ...
    public <T> T processArchiveEntryContent(FileContentToProcess fileContentToProcess, FileContentProcessor<T> fileContentProcessor)
        ...
        GetObjectRequest request = GetObjectRequest.builder()
                                                   .bucket(bucketName)
                                                   .key(id)
                                                   .range(String.format(ObjectStoreConstants.BYTES_RANGE_FORMAT,
