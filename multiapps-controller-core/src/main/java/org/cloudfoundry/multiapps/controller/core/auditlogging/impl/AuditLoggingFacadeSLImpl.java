...
public class AuditLoggingFacadeSLImpl implements AuditLoggingFacade {
    ...
    @Override
    public void logSecurityIncident(CustomAuditLog configuration) {
        writeMessage(auditLogManager.getSecurityLogger(), configuration.getPerformedAction(), Level.WARN);
    }

    @Override
    public void logDataAccessAuditLog(CustomAuditLog configuration) {
        writeMessage(auditLogManager.getConfigLogger(), configuration.getPerformedAction(), Level.WARN);
    }

    @Override
    public void logConfigurationChangeAuditLog(CustomAuditLog configuration, ConfigurationChangeActions configurationAction) {
        writeMessage(auditLogManager.getConfigLogger(), configuration.getPerformedAction(), Level.WARN);
    }
