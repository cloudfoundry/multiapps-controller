...
public class CloudControllerClientFactory {
    ...
    public CloudControllerClient createClient(OAuth2AccessTokenWithAdditionalInfo token) {
        ...
        CloudControllerRestClient controllerClient = clientFactory.createClient(configuration.getControllerUrl(),
                                                                                createCredentials(token),
                                                                                null, oAuthClient, exchangeFilters,
    ...
    public CloudControllerClient createClient(OAuth2AccessTokenWithAdditionalInfo token, String org, String space,
        ...
        CloudControllerRestClient controllerClient = clientFactory.createClient(configuration.getControllerUrl(),
                                                                                createCredentials(token),
                                                                                org, space, oAuthClient,
                                                                                exchangeFilters, requestTags);
        return new ResilientCloudControllerClient(controllerClient);
    }
    ...
    public CloudControllerClient createClient(OAuth2AccessTokenWithAdditionalInfo token, String spaceId,
        ...
        Map<String, String> requestTags = buildCorrelationIdTag(correlationId);
        CloudControllerRestClient controllerClient = clientFactory.createClient(configuration.getControllerUrl(),
                                                                                createCredentials(token), target,
    ...
    private Map<String, String> buildCorrelationIdTag(String correlationId) {
        return Optional.ofNullable(correlationId)
                       .map(correlationIdValue -> Map.of(TaggingRequestFilterFunction.TAG_HEADER_CORRELATION_ID,
                                                         correlationIdValue))
